...
> * The comma after `[START TIME]` can be replaced with `to`.
...
Searches and lists all tasks that contain the specified keyword, inclusive of `DAY`, `DATE`, `TIME`, `PRIORITY`.<br>
Format: `search KEYWORD`<br>

> * The search is not case sensitive.
> * The order of the keywords does not matter. e.g. `buy milk ntuc ` will match `ntuc buy milk`
> * GGist will show you all task that matches at least one of your search keywords e.g. `buy milk` will match `buy cow`
